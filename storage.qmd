...
[Rclone](https://rclone.org/) provides a convenient way of synchronising files `local -> cloud` or `cloud -> cloud`. Rclone keeps track of the transferred files and will retry if the connection is interrupted. **It is therefore the best option for users wishing to add large volumes of imagery to Sigma2**.
...
The quickest way to add SeaBee's MinIO storage as an rclone "remote" is to create and edit the `rclone.conf` file. Check the location of this file by running the command
...
The configuration setup can also be completed by following rclone's interactive configuration session, which can be started using
...
```txt
rclone copy -P -v "C:\path\to\my\data\flight_folder" seabee-minio:ntnu/2022/flight_folder
```

In this example, the `-P` and `-v` flags are optional: the first (`Progress`) prints the progress of the data transfer, while the second (`verbose`) prints additional details that may be useful for debugging.
