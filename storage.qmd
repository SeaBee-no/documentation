...
## Tips for Windows users
 ...
 3. Follow the common steps below to configure the connection to SeaBee's MinIO.
:::
...
```   
 ...
 where `<ACCESS_KEY_ID>` can be your user name or a service account, and `<SECRET_ACCESS_KEY>` is the accompanying password. Remember to save the changes to this file.
...
As an example, the following command would copy data from a local (Windows) system to a location on MinIO within NTNU bucket (assuming the user has "write" access)

```bash
rclone copy C:\path\to\my\data\flight_folder seabee-minio:ntnu/2022/flight_folder
```