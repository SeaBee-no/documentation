...
The data from each flight should be gathered into a **single folder** following the subfolder structure described below. The name of the flight folder itself can be anything you choose, but it must be unique and, from a user perspective, it helps to include standard information such as location, date etc. **The SeaBee platform does not enforce strict requirements for naming flight folders**, because we recognise that different pilots and organisations have their own conventions (see e.g. the issue [here](https://github.com/SeaBee-no/documentation/issues/23)). However, a recommended approach is to use `grouping_area_yyyymmddHHMM` or `org_yyyymmddHHMM_group_area_spec_elev`. Note the use of **underscores** (`_`) to separate each of the main components, while hyphens (`-`) or `CamelCase` can be used to further divide each part, if necessary. For example: `multipart-group_area-part-1_yyyymmddHHMM` or `MultipartGroup_AreaPart1_yyyymmddHHMM`.

::: {.callout-important}
The name of the flight folder is **not used** by any of the automated processing routines on the SeaBee platform. As long as the subfolders are arranged correctly and the details are correct in `config.seabee.yaml`, the mission will be processed regardless of the flight folder name. The name of the flight folder only matters for people browsing data manually (e.g. using MinIO). It is therefore important that names are human-readable. 
:::

In the recommended structure, the main components of the folder name have the following meanings:
 ...
 * `org` is the organisation collecting the data.

 * `spec` is the spectrum type e.g. RGB, multispectral or hyperspectral.

 * `elev` is the flight elevation (in metres).
